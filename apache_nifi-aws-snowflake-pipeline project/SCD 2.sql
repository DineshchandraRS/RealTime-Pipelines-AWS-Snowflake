...
select
    *
from
    customer_table_changes;
create
    or replace view v_customer_change_data as
select
    CUSTOMER_ID,
    FIRST_NAME,
    LAST_NAME,
        ...
        from
            (
                select
                    ...
                    UPDATE_TIMESTAMP
                from
                    SCD_DEMO.SCD2.customer_table_changes
                where
...
from
    customer_history ch
    inner join customer_table_changes ctc on ch.customer_id = ctc.customer_id
where
...
select
    *
from
    v_customer_change_data;
create
    or replace task tsk_scd_hist warehouse = COMPUTE_WH schedule = '1 minute' ERROR_ON_NONDETERMINISTIC_MERGE = FALSE as merge into customer_history ch -- Target table to merge changes from NATION into
...
insert
    -- Inserting a new CUSTOMER_ID and updating an existing one both result in an insert
    (
        CUSTOMER_ID,
        FIRST_NAME,
        LAST_NAME,
